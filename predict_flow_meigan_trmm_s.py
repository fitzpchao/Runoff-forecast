...
CSV_TRAIN  = "trmm_mg_m.csv"
CSV_TEST = "trmm_mg_m.csv"
...
def generate_data(fileCSV, start, end):
    ...
    for i in range(start, end - TIMESTEPS - 1 - PRE_TIME):
        t = np.hstack( df[i:i+TIMESTEPS+PRE_TIME, 0:INPUT_COLNUM])    
        t =np.append(t, df_y[i+TIMESTEPS-HIS_TIMESTEPS :i + TIMESTEPS])
        t_y = df_y[i+TIMESTEPS+PRE_TIME]
        X.append( [t.tolist()])
        y.append( [t_y])
...
# 进行训练
# 封装之前定义的lstm
regressor = SKCompat(learn.Estimator(model_fn=lstm_model, model_dir="Models/model_trmm_mg_s"))
# 生成数据
...
# test_y.shape = (354, 1)
tain_start = 0
if TIMESTEPS-HIS_TIMESTEPS<0:
    tain_start = HIS_TIMESTEPS - TIMESTEPS
train_X, train_y, MAX_X, MAX_Y ,MIN_Y= generate_data(CSV_TRAIN, tain_start, TRAINING_END)
...
writer = pd.ExcelWriter('waizhou3.xls')
...
fig.savefig('flow_meigan_trmm_s.png')
plt.show()
