...
# 进行训练
# 封装之前定义的lstm
regressor = SKCompat(learn.Estimator( model_fn=lstm_model, model_dir="Models/model_trmm_ljdpc23"))
# 生成数据
...
print( np.array(predicted, dtype=np.float32).shape)
print(test_y.shape)
# 计算MSE
'''np.reshape(test_y,(364,))
np.reshape(predicted,(364,))'''
...
mse2 = ((obsi- simi) ** 2).mean()
print("Mean Square Error is:%f" % mse2)
#计算 Ens 计算纳希效率系数
...
writer = pd.ExcelWriter('lijiaduliao1.xls')
...
fig.savefig('flow_lijiadu_trmmpc.png')
plt.show()
